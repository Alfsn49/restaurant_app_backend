...
def create_new_zona(zona: ZonaCreate, db: Session = Depends(get_db)):
    print("Datos recibidos", zona)
    existing_zona = get_zona_by_name(db, zona.name)

    if existing_zona:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="La zona ya existe")

    return create_zona(db, zona.name, zona.sucursal_id)


@router.get("/get-zona/{zona_id}", response_model=ZonaOut)
def get_zona(zona_id: str, db: Session = Depends(get_db)):
...
def list_zonas_by_sucursal(sucursal_id: str, db: Session = Depends(get_db)):
    return get_zona_by_sucursal(db, sucursal_id)


@router.put("/update-zona/{zona_id}", response_model=ZonaOut)
def update_existing_zona(zona_id: str, zona_data: ZonaUpdate, db: Session = Depends(get_db)):
    zona = get_zona_by_id(db, zona_id)

    if not zona:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Zona no encontrada")

    updated_zona = update_zona(db, zona_id, zona_data)
    return updated_zona
